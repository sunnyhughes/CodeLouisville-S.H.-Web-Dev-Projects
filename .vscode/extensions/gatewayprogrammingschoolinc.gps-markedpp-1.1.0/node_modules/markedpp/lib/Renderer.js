...
Renderer.prototype.references = function (refs) {
  ...
  refs.map(function (ref) {
    ...
    return ref;
  }).sort(Renderer.sortByTitle).forEach(function (ref) {
...
Renderer.prototype.tableOfContents = function (toc, options) {
  ...
  var renderLink = function renderLink(text, autoid) {
    return '[' + _this.sanitizeHeadings(text) + '](#' + autoid + ')';
  };

  var out = toc.filter(function (t) {
    ...
    if (!_this.options.numberedHeadings && options.numbered) {
    ...
    if (options.numbered) {
      ...
      if (_this.options.numberedHeadings) {
