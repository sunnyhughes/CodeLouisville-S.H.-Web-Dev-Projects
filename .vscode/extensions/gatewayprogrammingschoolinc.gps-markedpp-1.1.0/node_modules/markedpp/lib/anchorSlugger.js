...
var MODE = {
  ...
  MARKED: 'marked',
  PANDOC: 'pandoc',
  UNIFIED: 'unified' // CJK punctuations 。？！，、；：【】（）〔〕［］﹃﹄“”‘’﹁﹂—…－～《》〈〉「」

};
var RE_CJK = /[\u2014\u2018\u2019\u201c\u201d\u2026\u3001\u3002\u3008-\u300d\u3010\u3011\u3014\u3015\ufe41-\ufe44\uff01\uff08\uff09\uff0c\uff0d\uff1a\uff1b\uff1f\uff3b\uff3d\uff5e]/g;
var RE_ENTITIES = /&([a-z][a-z0-9]+|#[0-9]{2,4});/ig;
var RE_SPECIALS_DOT = /[./?!:[\]`,()*"';{}+=<>~\\$|#@&\u2013\u2014^]/g;
var RE_SPECIALS = /[/?!:[\]`,()*"';{}+=<>~\\$|#@&\u2013\u2014^]/g;
/**
 * basicGithubId
 * @private
...
function basicGithubId(text) {
  return text.toLowerCase().replace(/\s+/g, '-') // whitespaces
...
function getBitbucketId(text) {
  text = 'markdown-header-' + text.toLowerCase().replace(/\\(.)/g, function (_m, c) {
    return c.charCodeAt(0);
  }) // add escaped chars with their charcode
  .normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/[^\w\s-]/g, '').replace(/\s+/g, '-') // whitespace
...
function getPandocId(text) {
  text = text.replace(emojiRegex(), '') // Strip emojis
  .toLowerCase().trim().replace(/%25|%/ig, '') // remove single % signs
  .replace(RE_ENTITIES, '') // remove xml/html entities
...
function getMarkedId(text) {
  return entities.decode(text).toLowerCase().trim().replace(/[\u2000-\u206F\u2E00-\u2E7F\\'!"#$%&()*+,./:;<=>?@[\]^`{|}~_]/g, '').replace(/\s/g, '-');
}
/**
...
function slugger(header, mode) {
  ...
  var href = replace(asciiOnlyToLowerCase(header.trim()));
